...
> **업데이트 (2026-07):** 이 가이드는 라벨 없는 실데이터의 이상률·학습 시간 기준
> 비교입니다. 이후 라벨 있는 합성 결함 벤치마크(`evaluation/synthetic_fault_eval.py`)
> 에서 **Dense Autoencoder가 F1 0.93으로 확실히 우세**함이 확인되었습니다
> (Isolation Forest 0.82, LSTM-AE 0.80). 탐지 정확도 기준의 모델 선택은
> [README](../README.ko.md#벤치마크-결과)의 벤치마크 결과를 우선 참고하세요.
> 아래 내용은 추론 속도·설명 가능성·운영 비용 관점에서 여전히 유효합니다.
